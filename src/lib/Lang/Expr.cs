...
namespace lib.Lang
{
	public abstract class Expr : ICloneable
	{
		...
		private static Expr ParseCall(IEnumerator<string> tokens, string argName, string foldItemName, string foldAccName)
		{
			...
			else if (t == "not") res = new UnaryNot(parse());
			else if (t == "shl1") res = new UnaryShl1(parse());
			else if (t == "shr1") res = new UnaryShr1(parse());
			else if (t == "shr4") res = new UnaryShr4(parse());
			else if (t == "shr16") res = new UnaryShr16(parse());
			else if (t == "and") res = new BinaryAnd(parse(), parse());
			else if (t == "or") res = new BinaryOr(parse(), parse());
			else if (t == "xor") res = new BinaryXor(parse(), parse());
			else if (t == "plus") res = new BinaryPlus(parse(), parse());
